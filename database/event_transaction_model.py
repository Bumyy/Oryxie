...
class EventTransactionModel:
    ...
    async def get_balance(self, pilot_id: int) -> int:
        ...
        query = """SELECT COALESCE(SUM(amount), 0) AS balance 
                   ...
                   WHERE pilot_id = %s AND currency_name = %s AND (
                       (event_name = %s) OR
                       (event_name != %s AND reason NOT LIKE %s AND reason NOT LIKE %s AND reason NOT LIKE %s AND reason NOT LIKE %s)
                   )"""
        result = await self.db.fetch_one(query, (pilot_id, self.currency_name, self.event_name, self.event_name, '%PIREP%', 'Nuclear Reset%', 'Multiplier Fix%', 'test%'))
    ...
    async def check_duplicate(self, pilot_id: int, reason_pattern: str) -> bool:
        query = "SELECT id FROM event_transactions WHERE pilot_id = %s AND event_name = %s AND reason LIKE %s"
        result = await self.db.fetch_one(query, (pilot_id, self.event_name, reason_pattern))
    ...
    async def get_top_holders(self, limit: int = 3) -> List[Dict]:
        query = """SELECT p.callsign, SUM(et.amount) as total_cookies 
                   ...
                   WHERE p.status = 1 AND et.currency_name = %s AND (
                       (et.event_name = %s) OR
                       (et.event_name != %s AND et.reason NOT LIKE %s AND et.reason NOT LIKE %s AND et.reason NOT LIKE %s AND et.reason NOT LIKE %s)
                   )
                   GROUP BY et.pilot_id, p.callsign 
                   ORDER BY total_cookies DESC 
                   LIMIT %s"""
        return await self.db.fetch_all(query, (self.currency_name, self.event_name, self.event_name, '%PIREP%', 'Nuclear Reset%', 'Multiplier Fix%', 'test%', limit))
    ...
    async def process_pirep_reward(self, pirep_data: Dict, pilots_model, cookie_multiplier: int) -> bool:
        ...
        # Check for duplicate BEFORE calculating (V3 only)
        if await self.check_duplicate(pilot_id, f"PIREP Reward: #{pirep_id}%"):
            return False
