...
class QuestionView(View):
    def __init__(self, author_id: int, timeout: int, options: dict):
        ...
        for label in options.keys():
            self.add_item(Button(label=label, style=discord.ButtonStyle.secondary, custom_id=label))
...
class TrainingCog(commands.Cog, name="Training"):
    ...
    @app_commands.command(name="verify", description="Start the pilot verification process.")
    async def verify(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(embed=embed, view=self.InitialVerificationView(cog=self))

    class InitialVerificationView(View):
        def __init__(self, cog: "TrainingCog"):
            super().__init__(timeout=300)
            self.cog = cog

        @discord.ui.button(label="Verify", style=discord.ButtonStyle.green, custom_id="modal_verify_button")
        async def verify_button(self, interaction: discord.Interaction, button: Button):
            await interaction.response.send_modal(self.cog.VerificationModal(cog=self.cog))

    class VerificationModal(Modal, title="Infinite Flight Verification"):
        ifc_username = TextInput(label="Infinite Flight Community Username", required=True)
        ...
        async def on_submit(self, interaction: discord.Interaction):
            ...
            if data and data.get("errorCode") == 0 and data["result"]:
                user_info = data["result"][0]
                if user_info.get("atcRank", 0) > 1:
                    await interaction.response.send_message(
                        f"Welcome, {interaction.user.mention}! As an IFATC member, you do not need to take the written test.",
                    )
                else:
                    view = self.cog.StartTestView(cog=self.cog)
                    await interaction.response.send_message(
                        content=f"Hello {interaction.user.mention}. Your account has been verified. Please click below to start the written test.",
                        view=view,
                    )
            else:
                await interaction.response.send_message("Could not find a user with that IFC username.")
    ...
    class StartTestView(View):
        ...
        @discord.ui.button(label="Start Written Test", style=discord.ButtonStyle.primary, custom_id="start_written_test_now")
        async def written_test_button(self, interaction: discord.Interaction, button: Button):
            ...
            try:
                ...
                print(f"Error creating test session task: {e}")
                self.cog.active_tests.remove(user.id)
    ...
    async def run_test_session(self, interaction: discord.Interaction):
        ...
        start_message = await interaction.followup.send(content=f"The test for {user.mention} is starting in this channel!", ephemeral=False)
        test_messages.append(start_message)
        ...
        for i, q_data in enumerate(questions):
            ...
            # Send the image as a separate message first, if it exists
            if q_data.get("image"):
                image_message = await channel.send(q_data["image"])
                test_messages.append(image_message)
            
            embed = discord.Embed(title=f"Question {i + 1}/{len(questions)}", description=f"{q_data['text']}\n\n{options_text}", color=discord.Color.blue())
            
            # Change the wording for the timestamp
            message_content = f"Question closes {discord.utils.format_dt(end_time, style='R')}"
            
            q_view = QuestionView(author_id=user.id, timeout=q_data["timeout"], options=q_data["options"])
            q_message = await channel.send(content=message_content, embed=embed, view=q_view)
            test_messages.append(q_message)
            
            await q_view.wait()

            if q_view.value == q_data["correct"]:
                score += 1
                correct_message = await channel.send("✅ Correct!")
                test_messages.append(correct_message)
                await asyncio.sleep(2)
            else:
                wrong_answers.append(q_data["id"])
                correct_answer = q_data['options'][q_data['correct']]
                if q_view.value is None:
                    incorrect_message = await channel.send(f"❌ Time's up! The correct answer was **{q_data['correct']}**: {correct_answer}")
                else:
                    incorrect_message = await channel.send(f"❌ Incorrect. The correct answer was **{q_data['correct']}**: {correct_answer}")
                test_messages.append(incorrect_message)
        ...
        # Waits 5 minutes before deleting the test messages for review.
        await asyncio.sleep(300) 
        try:
            await channel.delete_messages(test_messages)
        except Exception as e:
            print(f"Could not bulk delete messages: {e}")

# This is the function that discord.py looks for to load a cog.
async def setup(bot: commands.Bot):
    # This check ensures your API manager from bot.py is available before loading
    if not hasattr(bot, 'if_api_manager'):
        print("ERROR: InfiniteFlightAPIManager is not attached to the bot. TrainingCog not loaded.")
        return
