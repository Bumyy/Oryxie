...
class DispatchClaimView(discord.ui.View):
    ...
    @discord.ui.button(label="Claim Request", style=discord.ButtonStyle.success, emoji="✋", custom_id="dispatch_claim_request")
...
class FlightClaimConfirmView(discord.ui.View):
    ...
    @discord.ui.button(label="Yes, Claim Flight", style=discord.ButtonStyle.success, emoji="✅", custom_id="confirm_claim_flight")
    async def confirm_claim(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            # Check permissions using database instead of roles
            error_msg = await self.flight_brain.check_pilot_permissions(
                str(interaction.user.id), self.flight_type, aircraft_code, 
                interaction.client.pilots_model, interaction.client.pireps_model
            )
            
            if error_msg:
                return await interaction.followup.send(error_msg, ephemeral=True)
            
            # Get pilot info for PDF
            rank, hours, error = await self.flight_brain.get_pilot_rank_and_aircraft(
                str(interaction.user.id), interaction.client.pilots_model, interaction.client.pireps_model
            )
            
            pilot_data = await interaction.client.pilots_model.get_pilot_by_discord_id(str(interaction.user.id))
            pilot_info = {
                'rank': rank,
                'callsign': pilot_data.get('callsign', 'QRV000') if pilot_data else 'QRV000'
            }
            ...
            # Generate PDF
            pdf_output = cog.pdf_service.generate_flight_pdf(self.flight_data, self.flight_type, interaction.user, pilot_info)
            ...
            if pdf_output:
                ...
                route = self.flight_data.route if isinstance(self.flight_data, FlightDetails) else self.flight_data.get('route', 'Unknown')
...
**{route}**"""
    ...
    @discord.ui.button(label="Cancel", style=discord.ButtonStyle.secondary, emoji="❌", custom_id="cancel_claim_flight")
...
# UI view for staff PDF regeneration
class StaffPDFRegenerateView(discord.ui.View):
    ...
    @discord.ui.button(label="Regenerate PDF (Staff Only)", style=discord.ButtonStyle.secondary, emoji="🔄", custom_id="staff_regenerate_pdf")
    async def regenerate_pdf(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Check if user has staff role
        user_roles = [role.name.lower() for role in interaction.user.roles]
        has_staff = any('staff' in role for role in user_roles)
        ...
        try:
            # Get cog and regenerate PDF
            cog = interaction.client.get_cog("FlightGeneratorPDF")
            if not cog:
                return await interaction.followup.send("❌ Service unavailable.", ephemeral=True)
            
            # Generate new PDF with same flight data
            pdf_output = cog.pdf_service.generate_flight_pdf(self.flight_data, self.flight_type, interaction.user)
            
            if pdf_output:
                # Send PDF as ephemeral message to staff
                pdf_buffer = io.BytesIO(pdf_output)
                flight_number = self.flight_data.flight_number if isinstance(self.flight_data, FlightDetails) else self.flight_data.get('flight_number', 'Unknown')
                
                await interaction.followup.send(
                    f"✅ PDF regenerated successfully for flight {flight_number}. You can forward this to the pilot if needed.",
                    file=discord.File(pdf_buffer, f"flight_{flight_number}_regenerated.pdf"),
                    ephemeral=True
                )
            else:
                await interaction.followup.send("❌ Failed to regenerate PDF.", ephemeral=True)
                
        except Exception as e:
            print(f"[DEBUG] Exception in regenerate_pdf: {e}")
            await interaction.followup.send("❌ An error occurred while regenerating the PDF.", ephemeral=True)
...
class AmiriApprovalView(discord.ui.View):
    ...
    @discord.ui.button(label="Approve", style=discord.ButtonStyle.success, emoji="✅", custom_id="amiri_approve")
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if amiri_channel:
            ...
            staff_view = StaffPDFRegenerateView(flight_data, "amiri", self.cog.bot.flightdata)
            
            # Send flight with both claim and staff regenerate buttons
            await amiri_channel.send(embed=embed, view=claim_view)
            await amiri_channel.send("Staff Tools:", view=staff_view)
    ...
    @discord.ui.button(label="Reject", style=discord.ButtonStyle.danger, emoji="❌", custom_id="amiri_reject")
...
class FlightGeneratorPDF(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.bot.add_view(DispatchClaimView(None, None, None, None))
    ...
    async def handle_flight_request(self, interaction: discord.Interaction, flight_type: str):
        # Check pilot permissions using database instead of roles
        rank, hours, error = await self.bot.flightdata.get_pilot_rank_and_aircraft(
            str(interaction.user.id), self.bot.pilots_model, self.bot.pireps_model
        )
        
        if error:
            return await interaction.response.send_message(error, ephemeral=True)
        
        # Special case for Ruby rank and amiri flights
        if rank == "Ruby" and flight_type == "amiri":
            await interaction.response.send_message(
                f"✅ Your flight request for an **A319-100** is sent. A dispatcher will soon announce new flights in `amiri-flights`. Keep an eye on the channel.", 
                ephemeral=True
            )
            await self.send_dispatch_request(interaction, flight_type, "A319")
            return

        available_aircraft = self.bot.flightdata.get_available_aircraft_by_rank(rank, flight_type)
        
        if not available_aircraft:
            return await interaction.response.send_message(
                f"❌ Your rank ({rank} - {hours:.1f}h) does not permit you to request {flight_type} flights. You need more flight hours.", 
                ephemeral=True
            )
    ...
    async def send_dispatch_request(self, interaction: discord.Interaction, flight_type: str, aircraft: str, continent: str = None):
        # Get pilot rank from database instead of roles
        rank, hours, error = await self.bot.flightdata.get_pilot_rank_and_aircraft(
            str(interaction.user.id), self.bot.pilots_model, self.bot.pireps_model
        )
        ...
        if dispatch_channel:
            ...
            embed.add_field(name="Rank", value=f"{rank} ({hours:.1f}h)" if not error else rank, inline=True)
            ...
            view = DispatchClaimView(interaction.user, flight_type, aircraft, rank, continent)
    ...
    async def executive_flight(self, interaction: discord.Interaction, aircraft: str, departure: str, arrival: str):
        ...
        if exec_channel:
            ...
            staff_view = StaffPDFRegenerateView(flight_data, "executive", self.bot.flightdata)
            
            # Send flight with both claim and staff regenerate buttons
            await exec_channel.send(embed=embed, view=claim_view)
            await exec_channel.send("Staff Tools:", view=staff_view)
